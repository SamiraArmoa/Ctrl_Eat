...
int guardar_productoIngredientes(int id_pr, char *ingredientes_ids){
	...
	while (id_ingrediente != NULL) {
		...
		guardar_productoIngrediente(id_pr, id_casteado, CLIENT_DB_PATH);
...
int cargar_ingredientes() {
	...
	while (fgets(line, sizeof(line), file)) {
		...
		if (id && nombre) {
			...
			guardar_ingredientes(id_casteado, trimmed_nombre, CLIENT_DB_PATH);
//			printf("%s,%s\n", id, nombre);
		} else {
...
int cargar_productos() {
	...
	while (fgets(line, sizeof(line), file)) {
		...
		if (id && nombre && ingredientes && tipo && precio && extra) {
			...
			while (len > 0 && (trimmed_precio[len - 1] == '\n' || trimmed_precio[len - 1] == '\r')) {
			...
			printf("%d, %s, %s, %f, %s, %s\n", id_casteado, nombre, tipo, precio_parseado, tamanio, alergenos);
			guardar_productos(id_casteado, nombre, tipo, precio_parseado,tamanio,alergenos, CLIENT_DB_PATH);
...
int cargar_restaurantes() {
	...
	while (fgets(line, sizeof(line), file)) {
		...
		if (id && nombre && ciudad) {
			...
			guardar_restaurantes(id, nombre, trimmed_ciudad, CLIENT_DB_PATH);
//			printf("%s, %s, %s\n", id, nombre, ciudad);
		} else {
			fprintf(stderr, "ERROR EN LA LINEA: %s\n", line);
		}
...
int cargar_csvs() {
	...
	printf("ha terminado con ingredientes\n");
	int ce2 = cargar_productos();
	printf("ha terminado con prodcucto\n");
