...
int insertarEmpleado(char *nombre, char *cargo, char *cuentaBancaria, int id_res, const char *path)
{
	int id;
	...
	sqlite3_bind_text(stmt, 4, cuentaBancaria, strlen(cuentaBancaria), SQLITE_STATIC);
...
int imprimirEmpleados(const char *path)
{
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW)
	{
		...
		printf("%i. Nombre: %s Cargo: %s Cuenta bancaria: %s ID del restaurante: %s\n", id_em, nombre,
			   cargo, cuentaBancaria, id_res);
	}
...
int updateEmpleados(int id_em, char *nombre, char *cargo, char *cuentaBancaria, int id_res, const char *path)
{
	sqlite3 *db;
	sqlite3_stmt *stmt;
	int rc;
	...
	rc += sqlite3_bind_text(stmt, 3, cuentaBancaria, strlen(cuentaBancaria), SQLITE_STATIC);
...
const char* imprimirProductos(const char *path)
{
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW)
	{
		...
		char buffer[256];
		sprintf(buffer, "%i. Nombre:%s Precio:%.2f Tipo:%s Tamanio:%s Alergenos:%s\n", id_pr, nombre, precio, tipo, tamanio, alergenos);		
